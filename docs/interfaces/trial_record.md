...
| `cue_at_inference` | (M, 4) float64 | 推論ごとに方策に渡した目標の手がかり `[k_obs, x, y, 旗]`（俯瞰画像から計算。旗は入力から外した学習でも記録する）。真値は同じ行動の番号のこま 2·k_obs の `cube_pos`（2026-09-26 追加、決裁 0057。手がかりのない学習のモデルでは (0, 4)） |
